...
function gimme(triplet) {
  // find the middle of the three by spreading the array in Math.max/min
  const middle = triplet.filter(
    (digit) => digit < Math.max(...triplet) && digit > Math.min(...triplet)
  );
  // return the index by changing the filtered result to an integer
